...
;(async()=>{
 ...
 try {
  ...
  await p.screenshot({path:'docs/qa-san-jose/login-exclusivo.png',fullPage:true})
  ...
  fs.writeFileSync('docs/qa-san-jose/exclusive-result.json',JSON.stringify({passed:true,checks:['identidad de San José','sin registro de restaurantes','ignora preferencia de otra empresa','rechaza cambio de empresa','acceso autorizado al módulo de distribución','usuario ajeno rechazado']},null,2))
